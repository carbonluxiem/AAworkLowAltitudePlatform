    ...
    begin: [
        {
            ...
            tags: ["科普", "入门"],
            time: "5 min",
            url: "https://www.bilibili.com/video/BV17y411b7eM/?spm_id_from=333.337.search-card.all.click&vd_source=d74182d8655b43705190f85140832331"
        },
        {
            title: "无人机基础知识图解",
...
window.onload = async function () {
    ...
    if (score) {
        ...
        document.getElementById("learningIntro").textContent =
            "你的测评得分为 " + score + " / 100，当前推荐重点学习「" + levelName + "」阶段内容，加粗边框标注的就是";
...
function renderSection(id, list, isRec) {
    ...
    list.forEach(function (c) {
        ...
        var titleHtml = c.url
            ? '<a href="' + c.url + '" target="_blank" class="course-link">' + c.title + '</a>'
            : c.title;
        html += '<div class="course-card' + (isRec ? " course-card-rec" : "") + '">'
            + '<div class="course-card-top">'
            + '<h4 class="course-title">' + titleHtml + recBadge + '</h4>'
